            ...
            <ul className="hidden md:flex items-center gap-6 text-black font-neue font-light uppercase">
              {navItems.map(item => (
                <li key={item.label} className="relative group">
                  <Link href={item.href} className="flex items-center gap-1 text-sm hover:text-primary">
                    {item.label}
                    {item.submenu && (
                  ...
                  {item.submenu && (
                    <ul className="absolute left-0 top-full mt-2 w-56 bg-white shadow-md opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                      {item.submenu.map(sub => (
                        <li key={sub.label}>
                          <Link href={sub.href} className="block px-4 py-2 text-sm hover:bg-black/5">
                            {sub.label}
                          </Link>
                        </li>
                      ))}
